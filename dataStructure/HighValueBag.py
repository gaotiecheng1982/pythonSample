def HighValueBag(quantity, weightAmount, weight, value):
    ...
    for i in range(1, quantity + 1):
        for curWgt in range(1, weightAmount + 1):
            ...
            if curWgt >= weight[i - 1] and resultTable[i][curWgt] < resultTable[i - 1][curWgt - weight[i - 1]] + value[
                        i - 1]:  # 不装价值大
                resultTable[i][curWgt] = resultTable[i - 1][curWgt - weight[i - 1]] + value[
                    i - 1]  # 前i-1个物品的最优解与第i个物品的价值之和更大
